...
export class Help extends Command {
  ...
  constructor(protected readonly client: Erisa) {
    super();
  }

  async main(ctx: Context) {
    if (!ctx.args.length) {
      const categories = this.client.extensions.commands.commandsByCategory
        .sort((a, b) => (a.category < b.category ? -1 : 1))
        .map(({ category, commands }) => ({
          category,
          commands: commands.filter((c) =>
            c.ownerOnly || c.hidden ? ctx.isBotOwner : true
          ),
        }))
      ...
      for (const { category, commands } of categories) {
        ...
        else paginator.addLine(`┝━ # ${category || "No category"} ━`);

        for (const command of commands.sort((a, b) =>
          a.name < b.name ? -1 : 1
        ))
          paginator.addLine(
            `│ ${command.name}: ${command.overview.slice(0, 80)}`
          );
      }
      ...
      try {
        ...
        await ctx.send(
          "I am unable to send you DMs. Perhaps you have me blocked or have DMs disabled?"
        );
      }
    } else {
      const command: Command | undefined = this.client.extensions.commands.get(
      ...
      if (command.aliases && command.aliases.length)
        paginator.addLine(`Aliases: ${command.aliases.join(", ")}`);
      ...
      if (command.description)
        paginator.addLines(command.description.split("\n"));

      // eslint-disable-next-line no-await-in-loop
      for (const page of paginator.pages()) await ctx.send(page);
    }
  }
}
