...
import commands, { Paginator, parseArgs, Holder } from "..";
import * as fakeCmds from "./commands";

chai.use(chaiAsPromised);

const normalAttach = (c: Erisa) =>
  c.usePairsArray(
    commands(c, { owner: "foo", prefixes: ["!", "?", /^foo (.+)/] })
  );
...
describe("Paginator", () => {
  it("should have `maxLength` be the provided length, minus the length of the suffix", () => {
    ...
    expect(new Paginator("```", "big ending").maxLength).to.equal(
      2000 - "big ending".length
    );
    expect(new Paginator("```", "big ending", 1000).maxLength).to.equal(
      1000 - "big ending".length
    );
  });
...
describe("parseArgs", () => {
  ...
  it("should split the provided text along spaces, and group quoted text together, ignoring the singular apostrophe", () => {
    const { cmd, args, suffix } = parseArgs(
      `foo bar "foobar fazbaz" lorem 'ipsum' I'm running out of things to write here.`
    );
    ...
    expect(suffix).to.equal(
      `bar "foobar fazbaz" lorem 'ipsum' I'm running out of things to write here.`
    );
  });
});
